...
import {BaseProvider, ClassProvider, InjectableParam, Provider, Token, ValueProvider} from '../models';
import {
...
export class DIContainer {
    ...
    addProvider<T>(provider: Provider<T>, container: DIContainer = this): void {
        ...
        const providerKey = (provider as BaseProvider<T>)['provide'] || provider;

        this._providersContainers.set(providerKey, container);

        if (!(provider as BaseProvider<T>)['multi']) {
    ...
    protected inject<T>(type: Token<T>): T | T[] {
